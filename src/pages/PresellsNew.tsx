...
// Schema de validação para Presell
...
const PresellsNew = () => {
  ...
  const fetchPresells = async () => {
    try {
      ...
      if (error) {
        ...
        const presellsData = (data || []).map((presell: any) => ({
          ...presell,
          bullet_points: Array.isArray(presell.bullet_points) ? presell.bullet_points : [],
        }));
        setPresells(presellsData);
      }
    } catch (error: any) {
      console.error("Erro ao carregar presells:", error);
      toast({
  ...
  const onSubmit = async (data: PresellFormData) => {
    try {
      ...
      console.error("Erro ao salvar presell:", error);
      toast({
  ...
  const handleEdit = (presell: Presell) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      console.error("Erro ao excluir presell:", error);
      toast({
        ...
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <div className="space-y-4">
            ...
            {loading ? (
              ...
              <Card>
                <CardContent className="pt-6">
                  <p className="text-center text-muted-foreground">Nenhum presell cadastrado</p>
                </CardContent>
              </Card>
            ) : (
              presells.map((presell) => (
                <Card key={presell.id}>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Checkout: {presell.checkouts?.name || "N/A"}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
